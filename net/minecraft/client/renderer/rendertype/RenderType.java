...
public class RenderType {
    ...
    private RenderType(String name, RenderSetup state) {
        ...
        this.outline = state.outlineProperty == RenderSetup.OutlineProperty.AFFECTS_OUTLINE
            ? state.textures.values().stream().findFirst().map(p_465667_ -> RenderTypes.OUTLINE.apply(p_465667_.location(), state.pipeline.isCull()))
            : Optional.empty();
    }
    ...
    public void draw(MeshData meshData) {
        ...
        try {
            ...
            if (meshData.indexBuffer() == null) {
                ...
                gpubuffer1 = this.state.pipeline.getVertexFormat().uploadImmediateIndexBuffer(meshData.indexBuffer());
