...
public class ModItems {
	...
	private static void onResourceReload(AddServerReloadListenersEvent event) {
		// Port note (1.21.11): AddReloadListenerEvent#addListener(PreparableReloadListener)
		// was replaced with AddServerReloadListenersEvent#addListener(Identifier, PreparableReloadListener).
		// The port keeps the cache-clear behaviour via a no-op listener that piggy-backs on the reload lifecycle.
		event.addListener(
				Identifier.fromNamespaceAndPath(Reliquary.MOD_ID, "mob_charm_definitions"),
				(preparationBarrier, resourceManager, prepareExecutor, applyExecutor) -> java.util.concurrent.CompletableFuture
						.runAsync(() -> {}, prepareExecutor)
