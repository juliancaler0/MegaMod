...
import com.ultra.megamod.lib.azurelib.common.cache.object.GeoQuad;
import com.ultra.megamod.lib.azurelib.common.cache.object.GeoVertex;
import com.ultra.megamod.lib.azurelib.common.model.AzBone;
import com.ultra.megamod.lib.azurelib.common.render.item.AzItemRendererPipelineContext;
...
public class AzModelRenderer<K, T> {
    ...
    protected void render(AzRendererPipelineContext<K, T> context, boolean isReRender) {
        ...
        if (!azureRenderLogged) {
            azureRenderLogged = true;
            int n = model == null ? -1 : model.getTopLevelBones().size();
            String names = "";
            if (model != null) {
                StringBuilder sb = new StringBuilder();
                for (var b : model.getTopLevelBones()) sb.append(b.getName()).append("(").append(b.getCubes().size()).append("c) ");
                names = sb.toString();
            }
            com.ultra.megamod.MegaMod.LOGGER.info("[AzRender] render() called: model={} topBones={} bones=[{}]", model, n, names);
        }
    ...
    private static boolean azureRenderLogged = false;

    /**
     * Renders the provided {@link AzBone} and its associated child bones
     */
    protected void renderRecursively(AzRendererPipelineContext<K, T> context, AzBone bone, boolean isReRender) {
        if (!azureRecursiveLogged) {
            azureRecursiveLogged = true;
            com.ultra.megamod.MegaMod.LOGGER.info("[AzRecurse] FIRST renderRecursively: bone={} hidden={} cubes={} children={}",
                bone.getName(), bone.isHidden(), bone.getCubes().size(), bone.getChildBones().size());
        }
    ...
    protected void renderCubesOfBone(AzRendererPipelineContext<K, T> context, AzBone bone) {
        if (bone.isHidden()) {
            if (!azureCubeLogged) com.ultra.megamod.MegaMod.LOGGER.info("[AzCube] SKIP hidden bone: {}", bone.getName());
            return;
        }

        var poseStack = context.poseStack();
        int cubeCount = bone.getCubes().size();
        if (!azureCubeLogged && cubeCount > 0) {
            azureCubeLogged = true;
            com.ultra.megamod.MegaMod.LOGGER.info("[AzCube] Rendering bone={} cubes={} buffer={}",
                bone.getName(), cubeCount, context.vertexConsumer());
        }
    ...
    private static boolean azureCubeLogged = false;
    private static boolean azureRecursiveLogged = false;
