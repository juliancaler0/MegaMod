...
public class AzArmorRendererPipeline extends AzRendererPipeline<UUID, ItemStack> {
    ...
    public void scaleModelForBaby(AzArmorRendererPipelineContext context, boolean isReRender) {
        ...
        // In 1.21.11, HumanoidModel no longer has baby scaling fields (scaleHead,
        // babyHeadScale, babyYHeadOffset, babyZHeadOffset, babyBodyScale, bodyYOffset).
        // Baby entity scaling is now handled by the render state system.
        if (currentSlot == EquipmentSlot.HEAD) {
            float headScale = 1.5f / 2.0f; // Default baby head scale ratio
            poseStack.scale(headScale, headScale, headScale);
        } else {
            float bodyScale = 1 / 2.0f; // Default baby body scale
            poseStack.scale(bodyScale, bodyScale, bodyScale);
            poseStack.translate(0, 1.5f / 16f, 0);
        }
    }
