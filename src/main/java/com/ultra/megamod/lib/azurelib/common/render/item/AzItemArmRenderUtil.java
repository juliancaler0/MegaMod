...
public class AzItemArmRenderUtil {
    ...
    public static void renderArmForBone(
        ...
        var playerEntityRenderer = (AvatarRenderer<?>) client.getEntityRenderDispatcher().getRenderer(client.player);
        var playerEntityModel = playerEntityRenderer.getModel();
        var skinObj = ((LocalPlayer) ClientUtils.getClientPlayer()).getSkin();
        // Use reflection to get the texture identifier from PlayerSkin since accessor name varies by version
        Identifier playerSkin;
        try {
            var textureMethod = skinObj.getClass().getMethod("texture");
            playerSkin = (Identifier) textureMethod.invoke(skinObj);
        } catch (Exception e) {
            try {
                var textureMethod = skinObj.getClass().getMethod("textureLocation");
                playerSkin = (Identifier) textureMethod.invoke(skinObj);
            } catch (Exception e2) {
                playerSkin = Identifier.withDefaultNamespace("textures/entity/player/wide/steve.png");
            }
        }
    ...
    private static void renderLeftArm(
        PoseStack poseStack,
        AzBone bone,
        net.minecraft.client.model.player.PlayerModel playerEntityModel,
        Identifier playerSkin,
        int packedLight,
        ...
        playerEntityModel.leftSleeve.render(
            poseStack,
            modelRenderer.getOrRefreshBufferRenderType(itemContext, bone, RenderTypes.entityTranslucent(playerSkin)),
            packedLight,
            OverlayTexture.NO_OVERLAY
        );
    }
