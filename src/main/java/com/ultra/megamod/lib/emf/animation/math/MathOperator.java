...
/**
 * Enum of all operator/puncutation tokens the expression lexer can emit.
 * <p>
 * Ported 1:1 from upstream; only the {@code isValidationPhase} flag has been rewired
 * to read from {@link EmfRuntime} (our per-thread runtime state) instead of the
 * upstream {@code EMFManager} singleton.
 */
public enum MathOperator implements MathComponent {
    ...
    DIVIDE {
        ...
        public float execute(MathComponent first, MathComponent second) {
            ...
            // If the divisor is a variable it may default to 0 during animation
            // validation. Matching upstream, we intercept and short-circuit so the
            // expression doesn't get invalidated for a transient zero.
            if (sec == 0 && !second.isConstant() && EmfRuntime.current().isValidationPhase()) {
