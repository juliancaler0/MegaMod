...
public class TextureBatchedRenderer {
	...
	private void emitSpriteTile(
			GuiGraphics context, TextureAtlasSprite sprite, GuiSpriteScaling.Tile tile,
			int x, int y, int width, int height,
			Vector4fc color
	) {
	...
	private void emitSpriteNineSlice(
			GuiGraphics context, TextureAtlasSprite sprite, GuiSpriteScaling.NineSlice nineSlice,
			int x, int y, int width, int height,
			Vector4fc color
	) {
	...
	private void emitSpriteStretch(
			GuiGraphics context, TextureAtlasSprite sprite,
			int x, int y, int width, int height,
			Vector4fc color
	) {
		emitTextureBatched(
	...
	public void draw(GuiGraphics context) {
		context.draw(vcp -> {
			for (var entry : batch.entrySet()) {
				var vc = vcp.getBuffer(RenderType.guiTextured(entry.getKey()));
