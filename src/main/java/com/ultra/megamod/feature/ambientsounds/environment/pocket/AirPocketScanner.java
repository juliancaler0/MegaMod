...
import com.ultra.megamod.feature.ambientsounds.util.SimpleQuadBitSet;

public class AirPocketScanner extends Thread {

    public static int SCAN_STEP_AMOUNT = 100;
    ...
    private final SimpleHashMapDouble<BlockState> foundCount = new SimpleHashMapDouble<>();
    private SimpleQuadBitSet sky = new SimpleQuadBitSet();
    private SimpleQuadBitSet skyTested = new SimpleQuadBitSet();
    ...
    protected void findState(BlockState state, int distance) {
        ...
        foundCount.addValue(state, factor);
    }
    ...
    protected void scan(Level level, int distance, BlockPosInspection pos) {
        ...
        if (hearThrough(level, state, pos)) {
            ...
            for (int i = 0; i < Direction.values().length; i++) {
                ...
                BlockPosInspection insPos = map.get(newPos);
                if (insPos != null)
                    insPos.add(direction.getOpposite());
                else {
                    insPos = new BlockPosInspection(newPos, direction.getOpposite());
                    map.put(insPos, insPos);
                }
                totalSize++;
            }
        } else {
