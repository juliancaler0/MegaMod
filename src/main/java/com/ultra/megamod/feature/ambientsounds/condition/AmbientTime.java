...
import com.ultra.megamod.feature.ambientsounds.util.SimpleLinearInterpolation;
...
public abstract class AmbientTime {
    ...
    public static class AmbientTimeSunAngle extends AmbientTime {
        ...
        private final SimpleLinearInterpolation interpolation;
        ...
        public AmbientTimeSunAngle(double... times) {
            ...
            List<SimplePair<Double, Double>> points = new ArrayList<>();
            points.add(new SimplePair<>(times[times.length - 2] - 360, times[times.length - 1])); // Add the last point to before the start to allow for smooth transition over 0, same is done for the end
            for (int i = 0; i < times.length; i += 2)
                points.add(new SimplePair<>(times[i], times[i + 1]));
            points.add(new SimplePair<>(times[0] + 360, times[1]));
            interpolation = new SimpleLinearInterpolation(points);
        }

        @Override
        public double value(AmbientEnvironment env) {
            return interpolation.valueAt(env.sunAngle);
        }
    ...
    public static class AmbientTimeHour extends AmbientTime {
        ...
        public AmbientTimeHour(double... times) {
            ...
            if (times[times.length - 2] < 24)
                points.add(new SimplePair<>(times[times.length - 2] - 24, times[times.length - 1])); // Add the last point to before the start to allow for smooth transition over 0, same is done for the end
            for (int i = 0; i < times.length; i += 2)
                points.add(new SimplePair<>(times[i], times[i + 1]));
            if (times[0] > 0)
                points.add(new SimplePair<>(times[0] + 24, times[1]));
            interpolation = new SimpleLinearInterpolation(points);
        }

        @Override
        public double value(AmbientEnvironment env) {
            return interpolation.valueAt(env.dayTimeHour);
        }
    ...
    public static class Serializer implements JsonDeserializer<AmbientTime>, JsonSerializer<AmbientTime> {
        ...
        public AmbientTime deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            ...
            if (object.has("type") && object.get("type").getAsString().equalsIgnoreCase("angle"))
                return new AmbientTimeSunAngle(array);
            return new AmbientTimeHour(array);
        }
