...
public class Pane extends UiRenderMacros
{
    ...
    public Pane(final PaneParams params)
    {
        ...
        // TODO: Port cursor attribute parsing when custom cursor textures are supported
        // For now, cursor stays at default
    }
    ...
    /**
     * Used mostly for overrides for default logics
     */
    ...
    /**
     * Returns the first Pane (depth-first search) of a given ID. if it matches the specified type.
    ...
    public void setWindow(final BOWindow w)
    {
        ...
        if (toolTipLines != null && !toolTipLines.isEmpty() && w != null)
        {
            final var builder = PaneBuilders.tooltipBuilder();
            for (final MutableComponent line : toolTipLines)
            {
                builder.appendNL(line);
            }
            builder.hoverPane(this).build();
            toolTipLines = null; // consumed
        }
    ...
    protected synchronized void scissorsStart(final PoseStack ms, final int contentWidth, final int contentHeight)
    {
        ...
        // The PoseStack maps to GUI-scaled coordinates, but glScissor needs framebuffer pixels.
        // Multiply by guiScale to convert from GUI-scaled to framebuffer pixel coordinates.
        int scissorsXstart = Mth.clamp((int) Math.floor(start.x() * guiScale), 0, fbWidth);
        int scissorsXend = Mth.clamp((int) Math.floor(end.x() * guiScale), 0, fbWidth);

        int scissorsYstart = Mth.clamp((int) Math.floor(start.y() * guiScale), 0, fbHeight);
        int scissorsYend = Mth.clamp((int) Math.floor(end.y() * guiScale), 0, fbHeight);
        ...
        org.lwjgl.opengl.GL11.glScissor(scissorsXstart, scissorsYstart, scissorsXend - scissorsXstart, scissorsYend - scissorsYstart);
    }
    ...
    protected synchronized void scissorsEnd(final BOGuiGraphics target)
    {
        ...
        if (!scissorsInfoStack.isEmpty())
        {
            ...
            org.lwjgl.opengl.GL11.glEnable(org.lwjgl.opengl.GL11.GL_SCISSOR_TEST);
            org.lwjgl.opengl.GL11.glScissor(info.xStart, info.yStart, info.xEnd - info.xStart, info.yEnd - info.yStart);
        }
        else
        {
