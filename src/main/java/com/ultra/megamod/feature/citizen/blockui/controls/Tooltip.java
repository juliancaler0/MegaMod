...
public class Tooltip extends AbstractTextElement
{
    ...
    public void drawSelfLast(final BOGuiGraphics target, final double mx, final double my)
    {
        ...
        if (!preparedText.isEmpty() && isEnabled())
        {
            ...
            // Draw tooltip background using simplified fill
            fill(ms, 0, 1, 1, height - 2, BACKGROUND_COLOR);
            fill(ms, 1, 0, width - 2, height, BACKGROUND_COLOR);
            fill(ms, width - 1, 1, 1, height - 2, BACKGROUND_COLOR);
