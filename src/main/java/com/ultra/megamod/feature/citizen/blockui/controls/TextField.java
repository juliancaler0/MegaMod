...
{
    // Helper methods replacing Screen static methods removed in 1.21.11
    ...
    private static boolean hasControlDown() {
        long w = GLFW.glfwGetCurrentContext();
        return GLFW.glfwGetKey(w, GLFW.GLFW_KEY_LEFT_CONTROL) == GLFW.GLFW_PRESS || GLFW.glfwGetKey(w, GLFW.GLFW_KEY_RIGHT_CONTROL) == GLFW.GLFW_PRESS;
    }
    private static boolean isCopy(int key) { return key == GLFW.GLFW_KEY_C && hasControlDown(); }
    private static boolean isCut(int key) { return key == GLFW.GLFW_KEY_X && hasControlDown(); }
    private static boolean isPaste(int key) { return key == GLFW.GLFW_KEY_V && hasControlDown(); }
    private static boolean isSelectAll(int key) { return key == GLFW.GLFW_KEY_A && hasControlDown(); }
    ...
    public void drawSelf(final BOGuiGraphics target, final double mx, final double my)
    {
        ...
        if (relativeSelectionEnd != relativeCursorPosition)
        {
            ...
            fill(target.pose(), selectionStartX, drawY - 1,
                selectionEndX - selectionStartX, 2 + mc.font.lineHeight,
                0x60, 0x60, 0xFF, 0xC0);
        }
    }
