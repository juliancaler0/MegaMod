...
/**
 * 3D witch-hat armor model. Geometry ported faithfully from the Reliquary
 * 1.21.x ref source. In 1.21.11 {@link HumanoidModel} is generic over a
 * {@link HumanoidRenderState} and {@code renderToBuffer} is final in the
 * Model base class — mesh + layer definition is all this class needs to
 * expose; rendering goes through the IClientItemExtensions extension plus
 * the vanilla armor-layer pipeline.
 */
public class WitchHatModel extends HumanoidModel<HumanoidRenderState> {
	...
	public void setupAnim(HumanoidRenderState state) {
		...
		// Only the head part carries visible geometry; everything else stays hidden.
		setAllVisible(false);
		head.visible = true;
	}
}
